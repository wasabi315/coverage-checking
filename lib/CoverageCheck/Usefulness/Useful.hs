...
import CoverageCheck.Syntax (Dataty(argsTy), Signature(dataDefs), Ty, Value(VCon), Values(VCons, VNil), inhab, inhabAt)
...
usefulConCase sig d c (MkUseful usvs)
  = case splitInstances (argsTy (dataDefs sig d) c) usvs of
        (us, vs) -> MkUseful (VCons (VCon c us) vs)
...
usefulWildMissCase sig d nonEmptyAxiom (Left ()) (MkUseful vs)
  = MkUseful (VCons (inhab sig nonEmptyAxiom d) vs)
usefulWildMissCase sig d nonEmptyAxiom (Right (MkNonEmpty c _))
  (MkUseful vs) = MkUseful (VCons (inhabAt sig nonEmptyAxiom d c) vs)
